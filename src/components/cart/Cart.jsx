...
const Cart = () => {
    const { cart, checkOut, isCheckOut } = useContext(AppContext)
    ...
    return (
        <div className="bg-white rounded-md p-6 w-[calc(30%-1rem)] h-max tb:w-full mb:w-full">
            ...
            <div className="flex flex-nowrap flex-col items-start justify-start">
                {cart.map((item, index) => <CartItem id={item.id} key={index} name={item.name} qty={parseFloat(item.qty)} price={parseFloat(item.price)} total={parseFloat(item.total)} thumbnail={item.thumbnail} isModel={false} ></CartItem>)}
            </div>
